...
class Pose_300W_LP(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Bin values
        bins = np.array(range(-99, 102, 3))  # 102左闭右开，所以只到99
        binned_pose = np.digitize([yaw, pitch, roll], bins) - 1  # 3度为1类，从-99到+99;只计算0-65共66类

        # Get target tensors
        labels = binned_pose
        cont_labels = torch.FloatTensor([yaw, pitch, roll])  # 回归
